# u1-afc-lite: instructions for AI assistants
...
This repo publishes one plugin: `afc-lite`, lightweight Automated Filament Changer support for the
Snapmaker U1's four toolheads, ported from the extended firmware's `31-feature-afc-lite`. It ships the
`AFC`, `AFC_unit`, and `AFC_lane` Klipper extras plus a config that maps the four lanes (E0-E3) to the
U1 extruders and filament motion sensors, and gcode macros for color, material, vendor, map, spool id,
weight, tool change, and lane/tool unload. It carries Python (the Klipper extras), not just config,
patches, and shell.
